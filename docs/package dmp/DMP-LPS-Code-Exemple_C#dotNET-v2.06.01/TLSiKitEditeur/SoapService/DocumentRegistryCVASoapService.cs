...
namespace TLSiKitEditeur.SoapService
{
    public class DocumentRegistryCVASoapService : IDisposable
    {
        ...
        {
            return RegistryStoredQuery("Resources\\template_FindDocumentCVA.xml");
        }


        public void Delete()
        {
            UpdateDocumentSet(CVAMessageBuilder.DeleteMessage());
        }


        public void UpdateDocumentSet(string PathToXML)
        {
            ...
            SubmitObjectsRequest request = XDSb_CVA.LoadRegistryXDSbRequest(PathToXML);
            var response = _service.DocumentRegistry_UpdateDocumentSet(request);
            CheckResponseOk(response);
        }
        public void UpdateDocumentSet(SubmitObjectsRequest req)
        {
            Logger.Log.Info($"Appel de la fonction : {System.Reflection.MethodBase.GetCurrentMethod().Name}");
            _service.AddHeaders("urn:ihe:iti:xds-b:2007:UpdateDocumentSet-b");
        ...
        public RegistryStoredQueryResponse RegistryStoredQuery(string PathToXML)
        {
            ...
            if (CheckResponseOk(RegistryStoredQueryResponse))
            {
                ...
                {
                    // Recherche du node correspondant à l'historique de vaccination 87273-9
                    foreach (ClassificationType ct in obj.Classification)
                    {
                        if ("87273-9".Equals(ct.nodeRepresentation))
                        {
