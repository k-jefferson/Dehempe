...
namespace TLSiKitEditeur.SoapService
{
    public class DocumentRegistrySoapService : IDisposable
    {
        ...
        public void Mask()
        {
            UpdateDocumentSet(MessageBuilder.MaskMessage());
        }

        public void Delete()
        {
            UpdateDocumentSet(MessageBuilder.DeleteMessage());
        }

        public void Archive()
        {
            UpdateDocumentSet(MessageBuilder.ArchiveMessage());
        }
        ...
        public void UpdateDocumentSet(string PathToXML)
        {
            ...
            SubmitObjectsRequest request = XDSb.LoadRegistryXDSbRequest(PathToXML);
        ...
        public RegistryStoredQueryResponse RegistryStoredQuery(string PathToXML)
        {
            ...
            if (CheckResponseOk(RegistryStoredQueryResponse))
            {
                ...
                foreach (var obj in RegistryStoredQueryResponse.RegistryObjectList)
                {
                    ...
                    {
                        //for (ClassificationType lst : ct.getClassification()) {
                        //for(int i=0;i<ct.size();i++) {
                        //logger.info("node repr :" +ct.getNodeRepresentation());
